...
void MMult1(long m, long n, long k, double *a, double *b, double *c) {
  ...
  for (long J = 0; J < n/BLOCK_SIZE; J++) {
    for (long P = 0; P < k/BLOCK_SIZE; P++) {  
      for (long j = ((J)*BLOCK_SIZE); j < (J+1)*BLOCK_SIZE; j++) {
        for (long p = ((P)*BLOCK_SIZE); p < (P+1)*BLOCK_SIZE; p++) {
          for (long I = 0; I < m/BLOCK_SIZE; I++) {
            for (long i = ((I)*BLOCK_SIZE); i < (I+1)*BLOCK_SIZE; i++) {
...
int main(int argc, char** argv) {
  ...
  for (long p = PFIRST; p < PLAST; p += PINC) {
    ...
    double time = t.toc();
    double flops = m*n/1e9*k*2*NREPEATS/time; // TODO: calculate from m, n, k, NREPEATS, time
    double bandwidth = m*n/1e9*k*4*NREPEATS*sizeof(double)/time; // TODO: calculate from m, n, k, NREPEATS, time
