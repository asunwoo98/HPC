...
//defined M_PI here since g++ compiler seems to be unable to access M_PI
...
int main() {
  ...
  for (long i = 0; i < N; i++) {
    ...
    //Since i am on windows I dont have drand48(). Replace with below
    x[i] = ((double)rand()/RAND_MAX-0.5) * M_PI/2; // [-pi/4,pi/4]
    sinx_ref[i] = 0;
    sinx_taylor[i] = 0;
    sinx_intrin[i] = 0;
    sinx_vector[i] = 0;
